import {getVoidLogger, PluginEndpointDiscovery} from "@backstage/backend-common";
...
describe('catalogInfoGenerator', () => {
    ...
    beforeAll(() => {
        ...
        mockDiscovery = {
            getBaseUrl: (pluginId: string) => Promise.resolve(`${mockBaseUrl}/my-${pluginId}`),
            getExternalBaseUrl: (pluginId: string) => Promise.resolve(`${mockExternalBaseUrl}/my-${pluginId}`),
        };
    ...
    it('should return a catalog url if no main branch is set', () => {
        ...
        expect(catalogInfoGenerator.getCatalogUrl(repoUrl)).toBe(`${repoUrl}/blob/main/catalog-info.yaml`);
    });

    it('should return appropriate catalog url for both repo and default branch', () => {
        const repoUrl = "https://ghe.example.com/my-org/my-repo";
        const defaultBranch = "dev";
        expect(catalogInfoGenerator.getCatalogUrl(repoUrl, defaultBranch)).toBe(`${repoUrl}/blob/${defaultBranch}/catalog-info.yaml`);
    });

    it('should fail to return a default catalog-info yaml string if a wrong repo URL is set', async () => {
        await expect(catalogInfoGenerator.generateDefaultCatalogInfoContent('xxxyyy')).rejects.toThrow('URL parsing failed');
    });

    it('should return a default catalog-info yaml string if analysis is not set', async () => {
        const repoUrl = "https://ghe.example.com/my-org/my-repo";
        await expect(catalogInfoGenerator.generateDefaultCatalogInfoContent(repoUrl, false)).resolves.toBe(getDefaultCatalogInfo("my-org", "my-repo"))
    });
    ...
    it('should return a default catalog-info yaml string if analyze-location endpoint is not available', async () => {
        ...
        await expect(catalogInfoGenerator.generateDefaultCatalogInfoContent(repoUrl)).resolves.toBe(getDefaultCatalogInfo("my-org-2", "my-repo-2"))
    });
    ...
    it('should return a default catalog-info yaml string if analyze-location endpoint returns nothing', async () => {
        ...
        await expect(catalogInfoGenerator.generateDefaultCatalogInfoContent(repoUrl)).resolves.toBe(getDefaultCatalogInfo("my-org-3", "my-repo-3"))
        expect(fetch).toHaveBeenCalledWith(`${mockBaseUrl}/my-catalog/analyze-location`, {
            headers: {
    ...
    it('should return catalog-info yaml string if analyze-location endpoint returns some data', async () => {
        fetch.mockReturnValue(
            Promise.resolve({
                json: () => Promise.resolve(mockAnalyzeLocationResponse("my-org-4", ["my-repo-comp-41", "my-repo-comp-42"]))
            })
        );

        const repoUrl = "https://github.com/my-org-4/my-repo-4";
        await expect(catalogInfoGenerator.generateDefaultCatalogInfoContent(repoUrl)).resolves.toBe(`---
...
function getDefaultCatalogInfoWithoutSeparators(org: string, name: string): string {
...
function mockAnalyzeLocationResponse(org: string, componentsToReturn: string[]) {
