...
export async function findAllImports(
    ...
    const repos = await findAllRepositories(logger, githubApiService, catalogApi, catalogInfoGenerator, false);
    for (const repo of repos) {
        if (!repo.url) {
            continue;
        }
        const catalogUrl = catalogInfoGenerator.getCatalogUrl(repo.url, repo.defaultBranch);
...
export async function createImportJobs(
    ...
    for (const req of importRequests) {
        ...
        const repoCatalogUrl = catalogInfoGenerator.getCatalogUrl(req.repository.url, req.repository.defaultBranch);
        const hasLocation = await verifyLocationExistence(catalogApi, repoCatalogUrl);
        if (hasLocation) {
            result.push({
                status: "ADDED",
                repository: {
        ...
        const prToRepo = await githubApiService.submitPrToRepo(logger, {
            ...
            gitUrl: gitUrl,
            catalogInfoContent: req.catalogInfoContent ?? (await catalogInfoGenerator.generateDefaultCatalogInfoContent(req.repository.url)),
...
export async function getImportStatus(
    ...
    catalogInfoGenerator: CatalogInfoGenerator,
    repoUrl: string,
    defaultBranch?: string): Promise<Components.Schemas.ImportStatus> {
